...
void tutte_embedding(TriangleMesh& mesh, const std::vector<int>& boundary_order, int nb_iter = NB_ITER) {
    int n = mesh.vertices.size();
    std::vector<Vector> new_positions(n, Vector(0, 0, 0));
    ...
    for (size_t i = 0; i < boundary_order.size(); ++i) {
        ...
        int next = boundary_order[(i + 1) % boundary_order.size()];
        perimeter += (mesh.vertices[curr] - mesh.vertices[next]).norm();
    }
    ...
    for (size_t i = 0; i < boundary_order.size(); ++i) {
        int curr = boundary_order[i];
        int next = boundary_order[(i + 1) % boundary_order.size()];
        double theta = 2.0 * M_PI * acc_length / perimeter;
        mesh.vertices[curr] = Vector(cos(theta), sin(theta), 0);
        is_boundary[curr] = true;
        acc_length += (mesh.vertices[curr] - mesh.vertices[next]).norm();
    }

    for (int iter = 0; iter < nb_iter; ++iter) {
        new_positions = mesh.vertices;
        for (int i = 0; i < n; ++i) {
            if (is_boundary[i]) continue;
            Vector avg(0, 0, 0);
            for (int j : adjacency[i]) {
                avg = avg + mesh.vertices[j];
            }
            new_positions[i] = avg / adjacency[i].size();
            new_positions[i][2] = 0;
        }
...
void draw_embedding(const TriangleMesh& mesh, const std::string& filename, int W = WIDTH, int H = HEIGHT) {
    ...
    for (const auto& tri : mesh.indices) {
        int ids[3] = {tri.vtxi, tri.vtxj, tri.vtxk};
        for (int i = 0; i < 3; ++i) {
            const Vector& a = mesh.vertices[ids[i]];
            const Vector& b = mesh.vertices[ids[(i + 1) % 3]];
            int x0 = (a[0] - xmin) / (xmax - xmin) * (W - 1);
            int y0 = H - 1 - ((a[1] - ymin) / (ymax - ymin) * (H - 1));
            int x1 = (b[0] - xmin) / (xmax - xmin) * (W - 1);
            int y1 = H - 1 - ((b[1] - ymin) / (ymax - ymin) * (H - 1));

            int dx = abs(x1 - x0), sx = x0 < x1 ? 1 : -1;
            int dy = -abs(y1 - y0), sy = y0 < y1 ? 1 : -1;
            int err = dx + dy, e2;
            while (true) {
                put_pixel(x0, y0, 0, 0, 0);
...
int main() {
    ...
    for (int i = 0; i < N; ++i) boundary.push_back(i);
    std::cout << 111 << std::endl;
    tutte_embedding(mesh, boundary);
    std::cout << 111 << std::endl;
    draw_embedding(mesh, "tutte_embedding.png");
    std::cout << 111 << std::endl;
